...
impl TryFrom<AntiVersorOddOnOrigin> for NullVersorEvenAtOrigin {
    type Error = String;
    fn try_from(anti_versor_odd_on_origin: AntiVersorOddOnOrigin) -> Result<Self, Self::Error> {
        ...
        if fail {
            let mut error = "Elements from AntiVersorOddOnOrigin do not fit into NullVersorEvenAtOrigin { ".to_string();
            error.push_str(error_string.as_str());
            error.push('}');
            return Err(error);
        }
