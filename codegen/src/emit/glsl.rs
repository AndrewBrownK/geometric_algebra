    ...
    match ast_node {
        ...
        AstNode::ClassDefinition { class } => {
            ...
            for (i, group) in class.grouped_basis.iter().enumerate() {
                ...
                collector.write_fmt(format_args!(" g{}_;\n", i))?;
            }
        ...
        AstNode::TraitImplementation { result, class, parameters, body } => {
            ...
            match parameters.len() {
                ...
                1 if result.name == "Into" => {
                    lower_camel_case(collector, &parameters[0].multi_vector_class().class_name)?;
                    collector.write_all(b"_")?;
                    lower_camel_case(collector, result.name)?;
                    collector.write_all(b"_")?;
                    lower_camel_case(collector, &result_type_name)?;
                }
                1 => {
                    lower_camel_case(collector, &parameters[0].multi_vector_class().class_name)?;
                    collector.write_all(b"_")?;
                ...
                2 => {
                    ...
                    lower_camel_case(collector, result.name)?;
                    collector.write_all(b"_")?;
                    lower_camel_case(collector, &parameters[1].multi_vector_class().class_name)?;
                }
                _ => unreachable!(),
            }
            collector.write_all(b"(")?;
